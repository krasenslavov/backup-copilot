...
const excludeScriptsCondition = ["**/bkpc-admin.js", "**/admin/**/*"];

// Project configuration
const project = "bkpc";
const projectUrl = "bkpc.local";
const projectPath = "./";
...
/**
 * Task to move font files to the build folder.
 */
function fonts() {
...
/**
 * Task to optimize and move image files to the build folder.
 */
function images() {
...
function styles() {
	return (
		src(paths.dev.scss)
			...
			.pipe(
				notify({
					message: "Styles `build` task completed successfully.",
					onLast: true,
					enabled: process.env.NOTIFY !== "false"
				})
			)
 ...
 */
function cleanBuild() {
	return del([paths.build.styles, paths.build.scripts, paths.build.images, paths.build.fonts]);
}
...
exports.build = series(cleanBuild, parallel(fonts, images, scripts, styles));

if (typeof webpackComponents === "function") {
	exports.build = series(cleanBuild, parallel(fonts, images, scripts, webpackComponents, styles));
}
