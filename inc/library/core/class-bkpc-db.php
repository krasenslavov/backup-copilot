...
if ( ! class_exists( 'BKPC_DB' ) ) {
	class BKPC_DB {
		...
		/**
		 * Create a database backup archive with optional find/replace functionality.
		 */
		public function create_db_archive( $backup_dir, $advanced_options = array(), $find = array(), $replace_with = array() ) {
			try {
				...
				// Windows fix: Convert localhost to 127.0.0.1 to avoid socket issues
				if ( 'localhost' === strtolower( $db_host ) && 'WIN' === strtoupper( substr( PHP_OS, 0, 3 ) ) ) {
					$db_host = '127.0.0.1';
				}
